...
public class Captcha extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            File file=new File(Datas.DIR+"GFXs/font.png");
            BufferedImage buffer = ImageIO.read(file);
            BufferedImage captcha=new BufferedImage(80, 16, BufferedImage.TYPE_INT_RGB);
            Graphics2D g = (Graphics2D)captcha.getGraphics();
            String captchaTexte="";
            int index;
            for(int i=0;i<5;i++) {
                index=(int)((double)(Datas.arrayChars.length-1)*Math.random());
                g.drawImage(buffer, i*16, 0, (i*16)+16, 16, index*16, 0, (index*16)+16, 16, null);
                captchaTexte+=Datas.arrayChars[index];
            }
            OutputStream os = response.getOutputStream();
            ImageIO.write(captcha, "PNG", os);
            os.close();
            String captchaCryptee=Objet.getEncoded(captchaTexte);
